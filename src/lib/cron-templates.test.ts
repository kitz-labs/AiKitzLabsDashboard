...
const tempDir = mkdtempSync(path.join(tmpdir(), 'kitz-cron-templates-test-'));
...
test('cron templates create/list/update/delete', () => {
  const created = createCronTemplate({
    ...
    job: { id: 'x', agentId: 'kitz', schedule: { expr: '0 9 * * 1-5' }, payload: { kind: 'agentTurn', message: 'hi' } },
  });
  ...
  const updated = updateCronTemplate({
    ...
    job: { id: 'y', agentId: 'kitz', payload: { kind: 'agentTurn', message: 'hello' } },
  });
