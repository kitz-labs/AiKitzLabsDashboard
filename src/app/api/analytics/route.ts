...
export async function GET(req: NextRequest) {
  ...
  const websiteIframeUrlRaw = process.env.KITZ_ANALYTICS_WEBSITE_IFRAME_URL;
  const socialIframeUrlRaw = process.env.KITZ_ANALYTICS_SOCIAL_IFRAME_URL;
  ...
  if (ga4PropertyId && (ga4ServiceAccountJson || ga4ServiceAccountJsonB64)) {
    ...
    website = {
      ...
      configured: false,
      error:
        "Not configured. Set GA4_PROPERTY_ID + GA4_SERVICE_ACCOUNT_JSON (recommended) or PLAUSIBLE_SITE_ID + PLAUSIBLE_API_KEY, or KITZ_ANALYTICS_WEBSITE_IFRAME_URL.",
    };
  }
