/**
 * Seed script — populates the Kitz dashboard SQLite database with realistic sample data.
 * Run with: npx tsx scripts/seed.ts
 */
import Database from 'better-sqlite3';
import path from 'path';

const DB_PATH = process.env.KITZ_DB_PATH || path.join(process.cwd(), 'kitz.db');
...
const posts = [
  { platform: 'x', format: 'short_post', pillar: 1, text: 'Most AI agents fail because they try to do everything. Kitz does one thing: turns your expertise into pipeline. Here\'s how 👇', status: 'published', daysBack: 0, imp: 4200, likes: 89, replies: 23, reposts: 31, saves: 15 },
  ...
  { platform: 'x', format: 'short_post', pillar: 5, text: 'Kitz update: Our AI agent now handles X engagement, content creation, and cold outreach autonomously. Building in public.', status: 'published', daysBack: 5, imp: 6800, likes: 156, replies: 44, reposts: 63, saves: 32 },
...
const signalData = [
  ...
  { type: 'brand_mention', user: '@indiemaker42', summary: 'Mentioned Kitz in a thread about AI-native marketing tools', rel: 'high', action: 'Thanked and engaged in thread', days: 1 },
...
const activityEntries = [
  ...
  { days: 5, h: 10, action: 'post', detail: 'Published Kitz update post', result: '6,800 impressions — highest single post' },
